...
if (isset ($_POST['cmd']) && $_POST['cmd'] == 'submit_all')
{
	...
	if (is_array($mQue)) {
		foreach ($mQue as $post_id)
		{
		  ...
		  twitme_notify_twitter ($post_id, $post);
		  $aSubmitted[] = $post_id;
		  sleep(2);
		}
	}
...
if (isset ($_POST['cmd']) && $_POST['cmd'] == 'resend')
{
	...
	twitme_notify_twitter ($_POST['postid'], $post, true);
	...
	if (isset ($_POST['username']) && isset ($_POST['password']))
	{
		switch ($_POST['cmd'])
		{
			case 'store':
			 ...
			 $sExcludeCats      = urldecode(trim($_POST['twitme_exclude_cats']));
			 $sPostEdits        = urldecode($_POST['twitme_publish_edits']);
			 ...
			 add_option('twitme_google_key', $_POST['twitme_google_key'], '', FALSE);
			 add_option('twitme_exclude_cats',serialize($sExcludeCats), '', FALSE);
			 add_option('twitme_publish_edits', $sPostEdits);
			break;
			...
			case 'update':
			 ...
			 update_option('twitme_google_key', $_POST['twitme_google_key']);
			 update_option('twitme_exclude_cats', serialize($sExcludeCats));
			 update_option('twitme_publish_edits', $sPostEdits);
